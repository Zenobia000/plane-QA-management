...
Taken from upstream #9512; the fix beside it was not. Upstream wraps the column with
``Cast(..., CharField())`` at each call site, this fork had already given the surrounding
``Concat`` an ``output_field`` -- same defect, same result, and porting theirs would have
rewritten eleven files to no effect. The test is implementation-agnostic, so it guards our
version as well as it guards theirs, which is the half worth carrying across.
