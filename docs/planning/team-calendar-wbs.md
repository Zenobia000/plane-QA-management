...
**4.4 為什麼是 email 而不是站內收件匣:** `NotificationViewSet` 有 `.filter(entity_name="issue")`,
而通知卡片點下去會開 issue peek overview。把請假通知放進去會是「建了資料、永遠不顯示,顯示了也是壞的」——
跟側邊欄那個坑同一類。要走站內收件匣得同時改後端過濾器和一個整體形狀就是 work item 的上游元件;
email 不需要動這兩處就能把事件送到不在看畫面的人手上。

站內的「等你決定」佇列回答的是「我該決定什麼」,前提是人有打開頁面。Email 補的正是這個前提。
