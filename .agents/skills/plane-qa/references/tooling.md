...
Two limits worth knowing before building a process on it: the browser UI neither displays nor sets the field, so anything classified here is invisible to someone working in the web app; and no list endpoint filters on it, so "show me every NFR" means listing work items and filtering client-side.
...
| `issue`      | `list --state --priority --per-page` · `create --name [--state-id --priority --description --requirement-kind --body <JSON>]` · `get --issue` · `update` / `transition --issue --state-id [--requirement-kind --body]` · `comment --issue --body` · `archive --issue`†      |
