...
- **Breadth and nature are separate axes.** The shipped types are Epic (level 0) / Feature (1) / Story (2) / Bug (2) / Task (3), workspace-wide; requirement nature is `Issue.requirement_kind` (`functional|quality|none`), writable on `/api/v1` only — no CLI flag, no MCP input, no UI. Never encode nature as a type or a project property: type already carries `level`, so the type count becomes breadth × nature (this workspace hit nine, and `converge_work_item_types` exists to undo it).
