...
| GET                     | `frontline/`                   | Intake grouped by the project's chosen dimension. `dimension: null` when none is marked — the signal to render nothing. `totals.reports` counts each report once and **is not the sum of the groups**; `totals.multi_attributed` says how many are counted more than once |
