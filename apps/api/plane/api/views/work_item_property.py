...
class WorkItemPropertyListCreateAPIEndpoint(BaseAPIView):
    ...
    def get(self, request, slug, project_id):
        return self.paginate(
            ...
            on_results=lambda properties: WorkItemPropertySerializer(
                properties, many=True, fields=self.fields, expand=self.expand
            ).data,
        )
...
class WorkItemPropertyDetailAPIEndpoint(BaseAPIView):
    permission_classes = [ProjectEntityPermission]
    ...
    def patch(self, request, slug, project_id, property_id):
        ...
        if next_kind != property_definition.kind and property_definition.values.filter(deleted_at__isnull=True).exists():
            return Response(
                {"error": "A property with values cannot change its kind."}, status=status.HTTP_400_BAD_REQUEST
            )
        serializer.save()
...
class WorkItemPropertyValueListAPIEndpoint(BaseAPIView):
    permission_classes = [ProjectEntityPermission]
    ...
    def get(self, request, slug, project_id, issue_id):
        ...
        return self.paginate(
            request=request,
            queryset=values,
            on_results=lambda property_values: WorkItemPropertyValueSerializer(
                property_values, many=True, fields=self.fields, expand=self.expand
            ).data,
        )


class WorkItemPropertyValueDetailAPIEndpoint(BaseAPIView):
    permission_classes = [ProjectEntityPermission]
    ...
    def delete(self, request, slug, project_id, issue_id, property_id):
        ...
        if property_definition.is_required:
            return Response(
                {"error": "A required work item property cannot be cleared."}, status=status.HTTP_400_BAD_REQUEST
            )
