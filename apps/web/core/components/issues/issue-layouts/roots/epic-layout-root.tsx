...
/**
 * The same layouts the work-item list renders.
 *
 * An epic is an ordinary work item whose type carries `is_epic`, so there is nothing about
 * a list of them that needs a different list, calendar or board. The layout components read
 * their store through `IssuesStoreContext`, so pointing that at the epic store is the whole
 * of the difference -- the scoping itself lives in `ProjectEpicsFilter`, which appends
 * `epic: true` to every fetch.
 */
