...
A quality requirement's pass mark lives on the **test case**, not the work item: `--case-type performance` plus `--threshold-metric "checkout P95 latency" --threshold-operator lt --threshold-value 2 --threshold-unit s`. Metric, operator and value go together or not at all (the unit is optional — a ratio or a count has none), and changing any of them publishes a new immutable version, because the threshold is part of the contract. **Nothing evaluates a result against it yet**: results carry no measured value, so pass/fail is still the caller's assertion and there is no trend. The columns exist so that judgement has something to compare against, not because it is happening.
...
| `case`       | `list [--search --folder-id --issue-id --per-page]` · `create --title [--case-type --threshold-metric --threshold-operator --threshold-value --threshold-unit ...]` · `get/update/version/archive/link-issue/unlink-issue` · `attachments --case` · `attach --case --file --mime-type` · `detach --case --attachment`† |
...
| Cases      | `test_case_list`, `test_case_get`, `test_case_create`, `test_case_update` (both take `case_type?` and `threshold_metric?`/`threshold_operator?`/`threshold_value?`/`threshold_unit?`), `test_case_version_get`, links/archive, `test_case_attachment_list`, `test_case_attachment_upload` (base64), `test_case_attachment_delete`‡                                                                                                                                                                                                                                                                                                              |
