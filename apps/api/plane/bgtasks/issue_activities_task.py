...
def track_type(
    ...
    if current_type_id != requested_type_id:
        old_type = IssueType.objects.filter(pk=current_type_id, workspace_id=workspace_id).first()
        new_type = IssueType.objects.filter(pk=requested_type_id, workspace_id=workspace_id).first()
        issue_activities.append(
            IssueActivity(
                issue_id=issue_id,
                actor_id=actor_id,
