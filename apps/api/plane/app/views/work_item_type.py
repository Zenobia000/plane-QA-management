...
class WorkItemTypeListCreateEndpoint(BaseAPIView):
    """List or create workspace-wide work-item type definitions for the web app."""

    permission_classes = [WorkspaceEntityPermission]

    def get_queryset(self):
        return IssueType.objects.filter(workspace__slug=self.kwargs["slug"], deleted_at__isnull=True).order_by(
            "level", "name"
        )
    ...
    def get(self, request, slug):
        return self.paginate(
            ...
            on_results=lambda types: (
                WorkItemTypeSerializer(types, many=True, fields=self.fields, expand=self.expand).data
            ),
        )
    ...
    def post(self, request, slug):
        ...
        if IssueType.objects.filter(
            ...
            return Response(
                {"error": "A work item type with this name already exists."},
...
class WorkItemTypeDetailEndpoint(BaseAPIView):
    ...
    def patch(self, request, slug, type_id):
        ...
        if (
            ...
            and IssueType.objects.filter(
                workspace=work_item_type.workspace,
                name__iexact=name,
                deleted_at__isnull=True,
            )
            .exclude(pk=work_item_type.pk)
            .exists()
        ):
            return Response(
                {"error": "A work item type with this name already exists."},
...
class ProjectWorkItemTypeListCreateEndpoint(BaseAPIView):
    ...
    def get(self, request, slug, project_id):
        return self.paginate(
            ...
            on_results=lambda types: (
                ProjectWorkItemTypeSerializer(types, many=True, fields=self.fields, expand=self.expand).data
            ),
        )
    ...
    def post(self, request, slug, project_id):
        ...
        if not created:
            return Response(
                {"error": "Work item type is already enabled for this project."},
                status=status.HTTP_409_CONFLICT,
            )
        if serializer.validated_data.get("is_default", False):
            ProjectIssueType.objects.filter(project_id=project_id, deleted_at__isnull=True).exclude(
                pk=project_type.pk
...
class ProjectWorkItemTypeDetailEndpoint(BaseAPIView):
    ...
    def get_object(self):
        return ProjectIssueType.objects.select_related("issue_type").get(
            pk=self.kwargs["project_type_id"],
            workspace__slug=self.kwargs["slug"],
            project_id=self.kwargs["project_id"],
            deleted_at__isnull=True,
        )
    ...
    def patch(self, request, slug, project_id, project_type_id):
        ...
        serializer = ProjectWorkItemTypeSerializer(
            project_type,
            data=request.data,
            partial=True,
            context={"workspace_id": project_type.workspace_id},
        )
        serializer.is_valid(raise_exception=True)
        if (
            "issue_type" in serializer.validated_data
            and serializer.validated_data["issue_type"] != project_type.issue_type
        ):
            return Response(
                {"error": "The type link cannot be changed. Create a new link instead."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if serializer.validated_data.get("is_default", False):
            ProjectIssueType.objects.filter(project_id=project_id, deleted_at__isnull=True).exclude(
    ...
    def delete(self, request, slug, project_id, project_type_id):
        ...
        if Issue.objects.filter(
            project_id=project_id,
            type_id=project_type.issue_type_id,
            deleted_at__isnull=True,
        ).exists():
            return Response(
